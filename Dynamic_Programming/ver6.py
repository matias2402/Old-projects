...
class karlstrom():
    ...
    def setup(self, **kwargs):
        ...
        self.age_punish = (1.005**12)
        self.age_reward = (1.007**12)
    ...
    def pension(self,ap,ra=65,m=1):
        if ra < 65:
            bonus = self.age_punish*(65-ra)
        elif ra > 65:
            bonus = self.age_reward*(ra-65)
        else:
            bonus = 1
        ba = self.BA*bonus*(self.m_bonus_pct[m])
        supplement = ba*self.supplement_pct
        atp = 0.6*ap*np.minimum(1,1)*self.BA
    ...
    def bellman(self):
        ...
        class sol: pass
        sol.vr = np.zeros([np.size(self.t_grid),self.ap_n, np.size(self.m_grid),np.size(self.ra_grid)])
        sol.vw = np.zeros([np.size(self.t_grid),self.w_n,self.ap_n, np.size(self.m_grid),np.size(self.ra_grid)])
        sol.ccp = np.zeros([np.size(self.t_grid),self.w_n,self.ap_n,np.size(self.m_grid),np.size(self.ra_grid)])
        ...
        for it in range(np.size(self.t_grid)-1,-1,-1): # from period 52 (column 51) until period 0, backwards
            ...
            # Loop over retirement age
            for ira, ra in enumerate(self.ra_grid):

                # Loop over AP
                for iap, ap in enumerate(sol.grid_ap):
                        
                    # Compute benefits for both marital status
                    b_um = self.pension(ap=ap, m=0,ra=ra)
                    b_m = self.pension(ap=ap, m=1,ra=ra)

                    # Compute value of retiring for both marital status
                    u_retire_um = self.utility_retire(t=self.t_max,b=b_um)
                    u_retire_m = self.utility_retire(t=self.t_max,b=b_m)

                    # Last period
                    if t == 102:
                        sol.vr[np.size(self.t_grid)-1,iap,0,ira] = u_retire_um
                        sol.vr[np.size(self.t_grid)-1,iap,1,ira] = u_retire_m
                        sol.vw[np.size(self.t_grid)-1,:,iap,:,ira] = 0
                        continue
                
                    # Forced retirement at age 70
                    if t >= self.forced_retire:
                        # EV for retire for both marrital statuses over 70
                        sol.vr[it,iap,0,ira] = u_retire_um + self.beta*(self.married_trans[0,0]
                            *np.log(0 + np.exp(sol.vr[it+1,iap,0,ira]))+self.married_trans[0,1]
                            *np.log(0 + np.exp(sol.vr[it+1,iap,1,ira])))
                        sol.vr[it,iap,1,ira] = u_retire_m + self.beta*(self.married_trans[1,0]
                            *np.log(0 + np.exp(sol.vr[it+1,iap,0,ira]))+self.married_trans[1,1]
                            *np.log(0 + np.exp(sol.vr[it+1,iap,1,ira])))

                        # EV for work for both marrital statuses over 70
                        sol.vw[it,:,iap,0,ira] = 0 + self.beta*(self.married_trans[0,0]*
                            np.log(np.exp(sol.vw[it+1,:,iap,0,ira])+np.exp(sol.vr[it+1,iap,0,ira]))+
                            self.married_trans[0,1]*np.log(np.exp(sol.vw[it+1,:,iap,1,ira])+np.exp(sol.vr[it+1,iap,1,ira])))
                        sol.vw[it,:,iap,1,ira] = 0 + self.beta*(self.married_trans[1,0]*
                            np.log(np.exp(sol.vw[it+1,:,iap,0,ira])+np.exp(sol.vr[it+1,iap,0,ira]))+
                            self.married_trans[1,1]*np.log(np.exp(sol.vw[it+1,:,iap,1,ira])+np.exp(sol.vr[it+1,iap,1,ira])))
                        continue

                    # No working if older than retirement age
                    if t >= ra:
                    ...
                    # No retirement if younger than retirement age
                    if t< ra:
                        # Retirement is an absorbing state(but cannot be chosen hence utility zero)
                        sol.vr[it,iap,0,ira] = 0 + self.beta*(self.married_trans[0,0]
                            *np.log(0 + np.exp(sol.vr[it+1,iap,0,ira]))+self.married_trans[0,1]
                            *np.log(0 + np.exp(sol.vr[it+1,iap,1,ira])))
                        sol.vr[it,iap,1,ira] = 0 + self.beta*(self.married_trans[1,0]
                            *np.log(0 + np.exp(sol.vr[it+1,iap,0,ira]))+self.married_trans[1,1]
                            *np.log(0 + np.exp(sol.vr[it+1,iap,1,ira])))
                ...
                # Compute ap next period outside wage loop
                    ...
                    for iw, w in enumerate(grid_W):
                        ...
                        EV_next_retire_if_work_um = np.interp(ap_next, sol.grid_ap[:],sol.vr[it+1,:,0,ira])
                        EV_next_retire_if_work_m = np.interp(ap_next, sol.grid_ap[:],sol.vr[it+1,:,1,ira])
                        # Create function taking ap and w and interpolates to vw
                        f_um = interpolate.interp2d(sol.grid_ap[:],sol.grid_W[:, it+1],sol.vw[it+1,:,:,0,ira])
                        f_m = interpolate.interp2d(sol.grid_ap[:],sol.grid_W[:, it+1],sol.vw[it+1,:,:,1,ira])
                        ...
                        #Update values
                        util_work = self.utility_work(w=w, t=t)
                        sol.vw[it,iw,iap,0,ira] = util_work + self.beta*(
                            self.married_trans[0,0]*logsum_um + self.married_trans[0,1]*logsum_m)
                        sol.vw[it,iw,iap,1,ira] = util_work + self.beta*(
                            self.married_trans[1,0]*logsum_um + self.married_trans[1,1]*logsum_m)
                        # Conditional Choice Prob.
                        sol.ccp[it,iw,iap,0,ira] = np.exp(sol.vw[it,iw,iap,0,ira])/(np.exp(sol.vw[it,iw,iap,0,ira])\
                                            + np.exp(sol.vr[it,iap,0,ira]))
                        sol.ccp[it,iw,iap,1,ira] = np.exp(sol.vw[it,iw,iap,1,ira])/(np.exp(sol.vw[it,iw,iap,1,ira])\
                                            + np.exp(sol.vr[it,iap,1,ira]))
        t1 = time.time()
        elapsed = t1-t0
